...
Ogni utente del controller può abilitare l'autenticazione a due fattori (2FA) per aumentare la sicurezza dell'account. Per abilitare 2FA, segui gli stessi passaggi documentati all'interno dell'interfaccia web del firewall: [NethSecurity UI 2FA](../installation/remote_access.md#2fa-section).
...
Tieni presente che le unità potrebbero non inviare informazioni aggiornate durante il processo di aggiornamento precedente alla versione dell'unità 1.3.0. Per aggiornare manualmente le informazioni, utilizza il pulsante \`guilabel\`:\`Sync unit info\` nel menu dell'unità.
...
3.  Segnala problemi: se riscontri comportamenti insoliti o errori, specialmente dopo aver visto un avviso di versione, segui la procedura di [risoluzione dei problemi](../best-practices/troubleshooting.md).
