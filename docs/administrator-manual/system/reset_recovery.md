...
If NethSecurity was installed through an in-place migration from NethServer 7, the factory reset will not restore the default configuration. Instead, the system will restart with all configurations migrated from NethServer 7.

:::

If you prefer a clean start, you can proceed with a new installation rather than using the factory reset.

Alternatively, you can follow these steps:

- Update your NethSecurity with a newer image (if present) or using a standard image downloaded from downloading.This will rewrite the ROM of your NethSecurity with a standard one, removing any migration files.
...
This commands will not delete the log partition from disk if it exists. If you need to delete the old partition please refer to the storage-section for more details.
