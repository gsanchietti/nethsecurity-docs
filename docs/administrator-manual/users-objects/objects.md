...
If a domain set is not used in any firewall rule, it will be marked as `unused` in the list. To see where a domain set is used, click on the `Show usages` link next to the set.
...
1.  Access the Add Domain Set Interface
    ...
    - Click on **Add somain set** button
