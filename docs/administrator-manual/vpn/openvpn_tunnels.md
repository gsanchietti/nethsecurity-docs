...
By default, OpenVPN tunnel instances created on NethSecurity are initialized with the following values:
...
On the **server side**, the modal shows certificate information for the CA, server, and client certificates. On the **client side**, it shows only the CA and client certificates.
...
2.  Click the **︙** menu on the right of the tunnel and select **Regenerate certificates**.
3.  Download the new client certificate and import it on the client side.

These operations will create new server and client certificates without affecting the CA certificate (which is assumed to be still valid in this case). In this scenario, using the new client certificate on the client firewall is **mandatory** to restore the connection, so make sure to download and import it on the client side as soon as possible to minimize downtime.
...
These commands will generate a new CA certificate, as well as new server and client certificates signed by the new CA. In this scenario, it is **mandatory** to download and import the new client configuration on the client side to restore the connection, so make sure to do it as soon as possible to minimize downtime.
