...
- `Traffic type`: Specifies which traffic the rule applies to.
  ...
  - `All traffic`: the rule applies to all incoming traffic regardless of protocol, which is forwarded to the configured destination IP without further filtering. When this option is selected, the form is reduced and only the destination IP address needs to be configured. Use this setting with caution, as it may expose the system to unintended or potentially harmful traffic.
...
- `Source port`: the port from which the request originates. Note that not all protocols require a port. For example, protocols like `GRE` do not use ports.
