...
- `Mac binding`:
  ...
  - `Type`: it\'s possible to choose between two types of MAC-IP binding:
    ...
    - `Strict binding`: Only hosts with a reservation allowed, others are blocked
      ...
      **Example**: A corporate network with strict security policies. Here hard binding ensures that only devices with a pre-configured reservation can access the network. This prevents that employees steal an IP with higher authorizations.
- `DHCP`:
...
- [OpenWrt DNS and DHCP documentation](https://openwrt.org/docs/guide-user/base-system/dhcp)
