...
Confirm the `--config` parameter points to the correct configuration file (e.g., `myvpn.ovpn`). Ensure all references (e.g., `myvpn`) match your intended VPN configuration.
...
Please remember that the interface name (referred to as `tunmyvpn` in the example) must not exceed 13 characters.
...
2.  Restart all active VPN tunnels. This command will stop all tunnels and fully restart only those with the enabled value set to 1: :
